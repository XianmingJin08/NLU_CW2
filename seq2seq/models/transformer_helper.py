...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        As the input sentences (tokens) might be padded in order to have the same length in that batch, 
        encoder_padding_mask is used for preventing encoder from taking into account (“paying attention”) to padding 
        tokens when processing inputs. 
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        Encoder attention only exists in the decoder, whereas self attention exists in both encoder and decoder. Self 
        attention performs multi-head attention mechanism by using source embedding (if in the encoder) or target 
        embedding (if in decoder) as its query, key and value. On the other hand, encoder attention uses encoder 
        output as its key and value, in addition to decoder states from self attention as its query. Therefore, 
        self attention requires query, key and value have equal sizes whereas encoder attention does not have this 
        constraint. 
        
        The difference bewteen key_padding_mask and attn_mask is: key_padding_mask is commonly used to avoid attending 
        to padding elements, whereas attn_mask is used so that the model cannot look ahead of the sequence 
        (seeing future words) when making the predicitons.
        
        We do not need give attn_mask here because the previous attention layer (self attend) in the decoder already 
        used it. The query computed by the previous attention already avoids attend to the future words. 
        '''
