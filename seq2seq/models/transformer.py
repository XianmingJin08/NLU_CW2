...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        of each word to the input embeddings of the Transformer. We cannot use only the embeddings similar to the 
        ones for LSTM because the self-attention mechanism in Transformer operates on its inputs without taking into 
        account the sequence ordering (permutation equivariant). Hence, we need to add positional embeddings so that 
        if the order of our input changes, the input to the self-attention layer (input embeddings + positional 
        embeddings) will also change. '''
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            We need it in the decoder because the decoder should generate a sequence word-by-word. This means that it 
            may only see previous inputs and current input words when generating prediction. If we do not mask future 
            input words, the decoder will not learn useful functions because it can see future inputs and hence, 
            learn only to copy the immediate future input (t+1) for the current prediction (t). In contrast, 
            we do not need the self_attn_mask in the encoder because it does not have a constraint to only attend up 
            to the current input. In order to learn good representation of its input (by taking into account both 
            right-to-left and left-to-right contexts), the encoder can have access to the whole input sequence. 
            
            We do not need a mask for incremental decoding because in incremental decoding, we feed the decoder 
            token-by-token. Hence, we do not provide the decoder with inputs from future time steps when it is 
            generating prediction for the current time step. 
        ...
        if not features_only:
            ...
            We need a linear projection after the decoder layers because we need to project the decoder output (
            forward_state) to a tensor having the same size of the vocabulary size. 

            The dimensionality of forward_state after this line would be the size of vocabulary (length of 
            dictionary). More precisely: [batch_size, tgt_time_steps, num_vocabulary] 
