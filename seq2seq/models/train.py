...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            The lines of code perform the following operations:
                1. First, we perform forward pass on the input batch through the instantiated model by passing 
                    Sample['src_tokens'] = the sentences (sequence of tokens) in the source language
                    Sample['src_lengths']  = the sentence lengths in the source language
                    Sample['tgt_inputs']  = the correct sentences in the target language
                    The forward pass produces output predictions which are stored in the variable output.
                2. After that, the average training loss per sentence (i.e. cross entropy loss) is computed based on the
                   output predictions and the gold labels (sample['tgt_tokens']).
                3. Next, we perform backpropagation (compute gradients of the loss function with respect to model 
                   parameters)  by calling "loss.backward()".
                4. Moreover, we also clip the gradient norms so that they do not exceed a certain threshold 
                   (default threshold = 4.0). This step is necessary for the stability of the training process, 
                   for instance, to avoid the exploding gradient problem.
                5. Next, we update the model parameters by calling "optimizer.step()"
                6. Lastly, we reset gradients to zero ("optimizer.zero_grad()") before continuing to the next input 
                   batch so that gradients are not accumulated across input batches, i.e. each parameter update is 
                   influenced only by instances in the corresponding input batch.
